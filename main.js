(function () {
  ...
  window.closeDrawer = function() { drawer.classList.remove('open'); overlay.classList.remove('open'); drawer.setAttribute('aria-hidden','true'); document.body.style.overflow=''; };
  ...
  setTimeout(() => { pageJustLoaded = false; }, 100);
  ...
  if (lcEl) {
    const lcIO = new IntersectionObserver(entries => {
      ...
      document.querySelectorAll('.d-fill').forEach(b => { b.style.width = b.dataset.w + '%'; });
      lcIO.disconnect();
    }, { threshold: 0.3 });
    lcIO.observe(lcEl);
  }
  ...
  if (masonry) {
    masonry.addEventListener('mouseover', e => {
      ...
      if (!pin) return;
      pinList.forEach(p => { p.style.opacity = p !== pin ? '.8' : ''; });
    });
    masonry.addEventListener('mouseleave', () => {
      pinList.forEach(p => { p.style.opacity = ''; });
    });
  }
